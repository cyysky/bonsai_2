@echo off
setlocal
rem ---- llama.cpp CUDA build for this machine (MSVC 2022 + Ninja + CUDA 12.9) ----
...
cd /d "%~dp0llama.cpp" || exit /b 1

set "CFG=-B build -G Ninja -DCMAKE_BUILD_TYPE=Release -DGGML_CUDA=ON -DCMAKE_CUDA_ARCHITECTURES=86 -DGGML_NATIVE=ON -DLLAMA_CURL=OFF"
...
echo Done. Binaries are in llama.cpp\build\bin
exit /b 0
